  ...
  return (
    <div className="flex flex-col">
      <PageHeader
        ...
        subtitle={`${overview.kpis.schoolsCount} écoles équipées · ${overview.kpis.totalStudents} élèves suivis`}
      />

      <div className="flex flex-col gap-6 px-[34px] py-[26px]">
        <div className="grid grid-cols-4 gap-3.5">
          <Kpi label="Écoles équipées" value={String(overview.kpis.schoolsCount)} />
          <Kpi label="Élèves suivis" value={String(overview.kpis.totalStudents)} />
          <Kpi label="Recouvrement moyen" value={`${overview.kpis.recoveryPct}%`} />
          <Kpi label="Écoles intégrées" value={String(overview.kpis.integratedCount)} tone="text-terracotta" />
        </div>
        ...
        <div className="grid grid-cols-[1.45fr_1fr] items-start gap-[18px]">
          <div className="flex flex-col gap-[18px] rounded-2xl border border-border bg-card p-[22px]">
            <div className="flex items-baseline justify-between gap-4">
              <div className="flex flex-col gap-0.5">
                <div className="font-serif text-lg font-semibold text-ink">Écoles équipées par région</div>
                <div className="text-[12.5px] text-ink-muted">Nombre d&apos;écoles membres par région</div>
              </div>
            </div>
            <div className="flex flex-col gap-[15px]">
              {overview.regions.map((r) => (
                <div key={r.name} className="flex flex-col gap-1.5">
                  <div className="flex items-baseline justify-between gap-3">
                    ...
                    <span className="text-[12.5px] text-ink-muted">{r.count} école(s)</span>
                  </div>
              ...
              {overview.regions.length === 0 && (
                <div className="text-sm text-ink-faint">Aucune école enregistrée pour l&apos;instant.</div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-[18px]">
            <div className="flex flex-col gap-3.5 rounded-2xl bg-green-deep p-[22px] text-[#E7DFCB]">
              <div className="font-serif text-lg font-semibold text-[#FBF7EE]">
                Recouvrement des mensualités
              </div>
            ...
            <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-[22px]">
              <div className="font-serif text-lg font-semibold text-ink">Signaux à traiter</div>
              {overview.alerts.length === 0 && (
                <div className="text-sm text-ink-faint">Aucun signal pour l&apos;instant.</div>
              )}
        ...
        <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-[22px]">
          <div className="flex items-baseline justify-between gap-4">
            <div className="font-serif text-lg font-semibold text-ink">
              Progression de la mémorisation · réseau
            </div>
            <div className="text-[12.5px] text-ink-muted">Part des élèves par palier de sourates mémorisées</div>
          </div>
