...
export default function SchoolsTable({ rows }: { rows: SchoolAggregate[] }) {
  ...
  async function contact(school: SchoolAggregate) {
    ...
    await consoleMessagingProvider.send({
      ...
      body: `Bonjour ${school.contactName ?? ""}, la fédération souhaite échanger avec vous.`,
    });
    setContacted(true);
    ...
    <div className="flex flex-col gap-4">
        ...
        {visible.map((s) => {
          ...
          return (
            <button
              key={s.id}
              onClick={() => setDrawer(s)}
              className="grid w-full grid-cols-[2.1fr_1.1fr_0.8fr_0.9fr_1.1fr_1fr] items-center gap-3 border-b border-[#EAE2CF] px-5 py-3.5 text-left text-[13.5px] last:border-b-0 hover:bg-[#F6F0E2]"
            >
              ...
              <span className="text-[12.5px] text-ink-muted">{relativeDate(s.lastActivity)}</span>
            </button>
          );
        })}
        {visible.length === 0 && (
          <div className="px-5 py-8 text-center text-sm text-ink-faint">Aucune école ne correspond.</div>
        )}
      </div>
      <div className="text-[12.5px] text-ink-faint">
        {visible.length} école(s) affichée(s) sur {rows.length}
      </div>

      {drawer && (
        <div className="fixed inset-0 z-20 flex justify-end bg-black/40" onClick={() => setDrawer(null)}>
          <div
            className="flex h-full w-[430px] flex-col gap-5 overflow-y-auto border-l border-border-input bg-paper p-[26px]"
          ...
          >
            <div className="flex items-start justify-between gap-3.5">
              <div className="flex flex-col gap-1">
                ...
                <div className="text-xs text-ink-muted">
                  {drawer.region} · responsable {drawer.contactName ?? "—"}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <DrawerStat k="Élèves" v={String(drawer.studentsCount)} />
              <DrawerStat k="Recouvrement" v={`${drawer.recoveryPct}%`} />
              <DrawerStat k="Statut" v={STATUS_LABEL[drawer.status]} />
              <DrawerStat k="Dernier paiement" v={relativeDate(drawer.lastActivity)} />
            </div>
      ...
      {contacted && (
        <div className="fixed bottom-7 left-1/2 -translate-x-1/2 rounded-[11px] bg-ink px-5 py-3 text-[13px] text-[#F5F1E6] shadow-lg">
          Message envoyé au responsable sur WhatsApp
        </div>
      )}
    </div>
  );
