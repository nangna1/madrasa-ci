...
export default function CoursEnDirectView({ students, date }: { students: Row[]; date: string }) {
  ...
  async function toggleAttendance(row: Row, present: boolean) {
    ...
    setBusyId(null);
    if (!synced) flash(t("Hors ligne · présence enregistrée, envoi à la synchronisation"));
  ...
  async function cycleSourate(row: Row) {
    ...
    setBusyId(null);
    if (synced) {
      if (next === "ok") flash(t("Sourate {name} validée · {student}", { name: sourate.name, student: row.fullName }));
      router.refresh();
    } else {
            ...
            {row.nextSourate && badge ? (
              ...
              >
                <div className="flex flex-col items-start gap-0.5">
                  <span className="text-[10px] uppercase tracking-[0.08em] text-ink-faint">{t("Prochaine sourate")}</span>
